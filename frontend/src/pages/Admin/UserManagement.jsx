...
// /* =========================================================================================
//    TAB 1: FACULTY DIRECTORY (PROFILES)
//    - Purpose: Add faculty details to the database (Name, Email, Dept).
//    - Does NOT create a login account.
//    ========================================================================================= */
...
//         alert("Faculty Profile Added to Directory!");
//         setShowForm(false);
...
//             <p style={{fontSize:'13px', color:'#666', margin:'5px 0 0'}}>Manage faculty identity and department allocation.</p>
//         </div>
...
//           <h4 style={{marginTop:0, color:'#AD3A3C'}}>Add New Profile</h4>
...
//             </div>
//             <div style={{flex:"1 1 200px"}}>
//                 <label style={labelStyle}>Email (Official)</label>
...
//                 <select style={inputStyle} value={profileForm.dept_id} onChange={e=>setProfileForm({...profileForm, dept_id:e.target.value})} required>
//                     <option value="">Select Dept</option>
//                     {depts.map(d => <option key={d.id} value={d.id}>{d.dept_code}</option>)}
//                 </select>
//             </div>
//             <div style={{flex:"1 1 150px"}}>
//                 <label style={labelStyle}>Auth Key (6 Digit)</label>
//                 <input style={inputStyle} maxLength="6" value={profileForm.auth_key} onChange={e=>setProfileForm({...profileForm, auth_key:e.target.value})} required />
//             </div>
...
//    - Purpose: Find profiles WITHOUT accounts and create logins for them.
//    ========================================================================================= */
...
//         // Filter only those who do NOT have a user_id
//         const pending = res.data.filter(f => !f.user_id);
...
//         e.preventDefault();
//         try {
//             await api.post("/admin/faculty-login", {
//                 faculty_profile_id: selectedFaculty.profile_id,
//                 password: password
//             });
//             alert(`Login created successfully for ${selectedFaculty.faculty_name}`);
...
//     return (
//       <>
//         <h3>Faculty User Activation</h3>
//         <p style={{fontSize:'13px', color:'#666', marginBottom:'20px'}}>
//             The following faculty members exist in the directory but <strong>do not have login credentials</strong>. 
//             Select one to create their account.
//         </p>
  
//         <div style={{display:'flex', gap:'20px'}}>
//             {/* LEFT: LIST OF PENDING ACCOUNTS */}
//             <div style={{flex:1}}>
//                 <table style={tableStyle}>
//                     <thead>
//                         <tr>
...
//                     </thead>
//                     <tbody>
//                         {pendingList.length > 0 ? pendingList.map(f => (
//                             <tr key={f.profile_id} style={{background: selectedFaculty?.profile_id === f.profile_id ? '#fff3cd' : 'transparent'}}>
//                                 <td style={tdStyle}>{f.faculty_name}</td>
//                                 <td style={tdStyle}>{f.email}</td>
//                                 <td style={tdStyle}>
//                                     <button onClick={() => setSelectedFaculty(f)} style={actionBtn}>
//                                         Select
//                                     </button>
//                                 </td>
//                             </tr>
//                         )) : (
//                             <tr><td colSpan="3" style={{...tdStyle, textAlign:'center', color:'green'}}>All faculty members have active accounts.</td></tr>
//                         )}
//                     </tbody>
//                 </table>
//             </div>

//             {/* RIGHT: CREATE LOGIN FORM */}
//             <div style={{width:'300px', background:'#f9f9f9', padding:'20px', borderRadius:'8px', border:'1px solid #eee', height:'fit-content'}}>
//                 <h4 style={{marginTop:0}}>Create Credentials</h4>
//                 {selectedFaculty ? (
//                     <form onSubmit={handleCreateLogin}>
//                         <div style={{marginBottom:'15px', fontSize:'13px'}}>
//                             <strong>For:</strong> {selectedFaculty.faculty_name}<br/>
//                             <strong>Username:</strong> {selectedFaculty.email}
//                         </div>
//                         <label style={labelStyle}>Set Password</label>
//                         <input type="password" style={inputStyle} value={password} onChange={e=>setPassword(e.target.value)} required placeholder="Enter password..." />
                        
//                         <div style={{display:'flex', gap:'5px', marginTop:'15px'}}>
//                             <button type="submit" style={{...primaryBtn, width:'100%'}}>Create Account</button>
//                             <button type="button" onClick={() => setSelectedFaculty(null)} style={{...primaryBtn, background:'#ccc', color:'#333'}}>Cancel</button>
//                         </div>
//                     </form>
//                 ) : (
//                     <p style={{color:'#999', fontStyle:'italic', fontSize:'13px'}}>Select a faculty member from the list to set their password.</p>
//                 )}
//             </div>
//         </div>
//       </>
...
//    - Existing logic for cascading dropdowns and promotion.
//    ========================================================================================= */
...
//   useEffect(() => { if (selectedDept) api.get("/admin/batches").then(res => setBatches(res.data.filter(b => b.dept_id === parseInt(selectedDept)))); else setBatches([]); }, [selectedDept]);
//   useEffect(() => { if (selectedBatch) api.get("/admin/sections").then(res => setSections(res.data.filter(s => s.batch_id === parseInt(selectedBatch)))); else setSections([]); }, [selectedBatch]);

//   const fetchStudents = async () => {
//     if (!selectedSection) return alert("Select a section");
//     setLoading(true);
//     try {
//       const res = await api.get(`/admin/students-by-filter`, { params: { section_id: selectedSection, semester } });
//       setStudents(res.data);
//     } catch (e) { console.error(e); } finally { setLoading(false); }
//   };

//   const handlePromote = async (studentId) => {
//     const pwd = prompt("Enter password for CR login:");
//     if (!pwd) return;
//     try {
//       await api.post("/admin/promote-cr", { student_id: studentId, password: pwd });
//       alert("Promoted!");
//       fetchStudents();
...
//       {loading ? <p>Loading...</p> : (
//         <table style={tableStyle}>
//             <thead><tr><th style={thStyle}>Roll</th><th style={thStyle}>Name</th><th style={thStyle}>Role</th><th style={thStyle}>Action</th></tr></thead>
//             <tbody>
//                 {students.map(s => (
//                     <tr key={s.id}>
//                         <td style={tdStyle}>{s.roll_number}</td>
//                         <td style={tdStyle}>{s.full_name}</td>
//                         <td style={tdStyle}>{s.role === 'cr' ? <span style={{color:'green', fontWeight:'bold'}}>CR</span> : 'Student'}</td>
//                         <td style={tdStyle}>{s.role !== 'cr' && <button onClick={()=>handlePromote(s.id)} style={actionBtn}>Promote</button>}</td>
//                     </tr>
//                 ))}
//             </tbody>
//         </table>
...
// const tabBtn = (active) => ({ 
//     padding: "10px 15px", border: "none", borderBottom: active ? "3px solid #AD3A3C" : "3px solid transparent", 
//     background: "none", cursor: "pointer", fontWeight: active ? "bold" : "normal", 
//     color: active ? "#AD3A3C" : "#666", fontSize: "14px", transition: "all 0.2s"
// });
...
// const actionBtn = { padding: "6px 12px", background: "#333", color: "white", border: "none", borderRadius: "4px", cursor: "pointer", fontSize:"12px" };
